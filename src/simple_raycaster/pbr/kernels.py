...
@wp.kernel(enable_backward=False)
def raycast_gbuffer_sparse_kernel(
    meshes: wp.array(dtype=wp.uint64),
    mesh_pos_w: wp.array(dtype=wp.vec3, ndim=2),
    mesh_quat_w: wp.array(dtype=wp.vec4, ndim=2),
    mesh_albedo: wp.array(dtype=wp.vec3, ndim=2),
    ...
    cam_pos_w: wp.array(dtype=wp.vec3, ndim=1),
    cam_quat_w: wp.array(dtype=wp.vec4, ndim=1),
    tiles: wp.array(dtype=wp.int32, ndim=3),
    tile_size: int,
    fx: float,
    fy: float,
    cx: float,
    ...
    """Sparse G-buffer over selected tiles. ``tiles`` is ``[N, T, 2]`` (tx, ty).

    Launch ``dim=(N, T*S*S)``. Flat pix index packs tiles then row-major locals.
    """
    n, pix = wp.tid()
    n_meshes = mesh_pos_w.shape[1]
    s = tile_size
